    ...
    form_timelines = form_timelines.as_json.map { |form_timeline|
      {
        **form_timeline.symbolize_keys,
        form_id: { id: form_timeline.symbolize_keys[:form_id], name: "#{Form.find(form_timeline.symbolize_keys[:form_id])[:name]} (#{Pokemon.find(Form.find(form_timeline.symbolize_keys[:form_id])[:pokemon_id])[:name]})" }
      }
    }
