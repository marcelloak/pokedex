class Api::CostumeCaughtTimelinesController < ApplicationController
  def index
    costume_caught_timelines = CostumeCaughtTimeline.all
  ...
  private
    def costume_caught_timeline_params
      params.permit(:caught, :attack, :defence, :stamina, :current_level, :target_level, :gender, :shadow, :purified, :shiny, :lucky, :charge_move_unlocked, :transferred, :current_costume_id, :target_costume_id, :fast_move_id, :first_charge_move_id, :second_charge_move_id, :original_owner_id, :current_owner_id)
    end
end
